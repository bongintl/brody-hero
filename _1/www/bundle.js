...
var { fit, center, map } = __webpack_require__(8);
var view = __webpack_require__(9);
var images = __webpack_require__(12);
...
view(gui => {
    ...
    return (win, offset) => images.map((image, i) => {
        ...
        var scale = map(Math.abs(i - config.focus), 0, images.length - 1, config.scale, config.scale + config.depth);

        var parallax = [size[0] * (scale - 1) * offset[0] * .5, size[1] * (scale - 1) * offset[1] * .5];
...
var center = ([srcW, srcH], [destW, destH]) => {
    ...
    return [(destW - srcW) / 2, (destH - srcH) / 2];
};
...
var mouse = __webpack_require__(11)();
...
module.exports = fn => {
    ...
    var View = {
        ...
        view: () => {
            ...
            var offset = [mouse.position[0] / win[0], mouse.position[1] / win[1]];
...
module.exports = [{
    url: '/img/BA.Web.TestStories.Channel4.Image.21.jpg',
    size: [1600, 900],
    scale: 1,
    contain: false
}, {
    url: '/img/parallax.1.svg',
    size: [1440, 900],
    scale: 1,
    contain: false
}, {
